...
export default function ChatWindow() {
    ...
    useEffect(() => {
        ...
        const handleChatFrozen = ({ conversationId }: { conversationId: number }) => {
        ...
        socket.on("chat_frozen", handleChatFrozen);
        ...
        return () => {
            ...
            socket.off("chat_frozen", handleChatFrozen);
        };
    }, [activeConversationId, addMessage, myUserId, updateMessageStatus, freezeConversation]);
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        socket.emit("typing", { conversationId: activeConversationId, userName: currentUser.name });
    ...
    return (
        <div className="flex flex-col h-full w-full bg-travel-bg relative">
            ...
            <div className="flex items-center justify-between p-4 bg-travel-card border-b border-travel-border shadow-sm z-30">
                <div className="flex items-center gap-3">
                    <button
                        ...
                        aria-label={isSelectionMode ? "Cancel selection" : "Close chat window"}
                    >
                            ...
                            if (msg.isSystemMessage) {
                                return (
                                    <div key={msg.id} className="flex justify-center my-4 w-full">
                                        <div className="bg-travel-card/60 dark:bg-zinc-800/60 text-travel-text-muted text-xs px-4 py-1.5 rounded-full text-center border border-travel-border/50 shadow-sm backdrop-blur-sm">
                                            {msg.content}
                                        </div>
                                    </div>
                                );
            ...
            {/* TYPING INDICATOR */}
            {typingUsers.length > 0 && !isSelectionMode && activeChat.isActive !== false && (
                <div className="px-4 py-1 bg-travel-card/50 text-xs text-travel-text-muted italic">
                    {typingUsers.join(", ")} {typingUsers.length === 1 ? 'is' : 'are'} typing...
                </div>
            )}

            {/* Input Bar OR Frozen Chat Warning */}
            {!isSelectionMode && activeChat.isActive !== false ? (
                ...
                <div className="p-4 bg-travel-card border-t border-travel-border flex justify-center items-center">
                    <p className="text-sm text-travel-text-muted italic bg-travel-bg px-6 py-2 rounded-full border border-travel-border">
                        Chat closed. This request was canceled.
                    </p>
                </div>
            ) : null}
        </div>
