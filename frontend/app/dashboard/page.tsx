...
export default function DashboardPage() {
    ...
    // Data States
    const [trips, setTrips] = useState<Trip[]>([]); // Hosted
    const [joinedTrips, setJoinedTrips] = useState<JoinedTrip[]>([]); // Joined
    const [loading, setLoading] = useState(true);

    // UI States
    const [viewMode, setViewMode] = useState<'HOSTING' | 'JOINING'>('HOSTING');
    ...
    const fetchDashboardData = useCallback(async () => {
        ...
        try {
            // Fetch Hosted Trips
            const hostedRes = await axiosAuth.get("/api/trips/user/me");
            setTrips(hostedRes.data);
        } catch (error) {
        ...
        try {
            // Fetch Joined Trips
            const joinedRes = await axiosAuth.get("/api/requests/my-requests");
            setJoinedTrips(joinedRes.data);
        } catch (error) {
    ...
    useEffect(() => {
        if (status === "unauthenticated") {
            ...
            // Tell the aggressive linter that this data fetch pattern is intentional and safe
    ...
    // --- JOINING STATS ---
    const pendingJoins = joinedTrips.filter(req => req.status?.toLowerCase() === 'pending').length;
    const upcomingAdventures = joinedTrips.filter(req => req.status?.toLowerCase() === 'accepted').length;
                ...
                <div className="mb-10">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                            ...
                            {/* View Toggle (HOSTING vs JOINING) */}
                            <div className="bg-travel-card border border-travel-border p-1 rounded-lg flex gap-1 shadow-sm">
                                <button
                                    onClick={() => setViewMode('HOSTING')}
                                    className={`px-4 py-1.5 text-sm font-bold rounded-md transition ${viewMode === 'HOSTING' ? 'bg-travel-accent text-white shadow-sm' : 'text-travel-text-muted cursor-pointer hover:text-travel-text'}`}
                                >
                                    Hosting
                                </button>
                                <button
                                    onClick={() => setViewMode('JOINING')}
                                    className={`px-4 py-1.5 text-sm font-bold rounded-md transition ${viewMode === 'JOINING' ? 'bg-travel-accent text-white shadow-sm' : 'text-travel-text-muted cursor-pointer hover:text-travel-text'}`}
                                >
                                    Joining
                                </button>
                            </div>
                        ...
                        {/* Filter Tabs (Only show when looking at Hosted trips) */}
                        {viewMode === 'HOSTING' && (
                            <div className="flex bg-travel-card border border-travel-border rounded-lg p-1 w-full md:w-auto">
                                {['ALL', 'UPCOMING', 'PAST'].map((f) => (
                                    <button
                                        ...
                                        className={`flex-1 md:flex-none px-6 py-2 text-xs font-bold rounded-md transition-all cursor-pointer ${filter === f
                                            ? 'bg-travel-bg text-travel-text shadow-sm border border-travel-border'
                    ...
                    {/* Stats Cards for JOINING trips */}
                    {viewMode === 'JOINING' && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="bg-travel-card p-5 rounded-xl border border-travel-border shadow-sm flex items-center gap-4 hover:-translate-y-1 transition-transform">
                                <div className="h-12 w-12 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center text-xl"><FiActivity /></div>
                                <div>
                                    <p className="text-2xl font-bold text-travel-text">{pendingJoins}</p>
                                    <p className="text-xs text-travel-text-muted uppercase font-semibold">Pending Requests</p>
                                </div>
                            </div>
                            <div className="bg-travel-card p-5 rounded-xl border border-travel-border shadow-sm flex items-center gap-4 hover:-translate-y-1 transition-transform">
                                <div className="h-12 w-12 rounded-full bg-green-100 text-green-600 flex items-center justify-center text-xl"><FiMapPin /></div>
                                <div>
                                    <p className="text-2xl font-bold text-travel-text">{upcomingAdventures}</p>
                                    <p className="text-xs text-travel-text-muted uppercase font-semibold">Upcoming Adventures</p>
                                </div>
                            </div>
                        </div>
                    )}
                                            ...
                                            <div className="flex-1 p-6 flex flex-col justify-center border-r-0 md:border-r border-travel-border">
                                                <div className="flex flex-col gap-1 mb-4">
                                                    <div className="flex items-center gap-3">
                                                        <h3 className="text-2xl font-bold text-travel-text">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <Link href={`/trips/${trip.id}`} className="px-4 py-2 bg-travel-accent/10 hover:bg-travel-accent/20 text-travel-accent text-xs font-bold uppercase tracking-wide rounded-lg transition-colors cursor-pointer">
                                                        View Details
                                                    </Link>
                                                    <button onClick={() => handleDeleteTrip(trip.id)} className="p-2 text-red-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer" title="Delete Trip">
                                                        <FiTrash2 size={18} />
                                                    </button>
                                                </div>
                                            </div>

                                            {/* Right: Requests Toggle */}
                                            <button onClick={() => setExpandedTripId(isExpanded ? null : trip.id)} className={`w-full md:w-64 border-t md:border-t-0 p-6 flex md:flex-col items-center justify-between md:justify-center gap-3 hover:bg-travel-bg/50 transition cursor-pointer text-left md:text-center group ${isExpanded ? "bg-travel-bg/50" : ""}`}>
                                                        ...
                                                        {trip.joinRequests.map((req) => (
                                                            <div key={req.id} className="flex justify-between items-center bg-travel-card p-4 rounded-xl border border-travel-border shadow-sm">
                                                                <div className="flex items-center gap-3 min-w-0">
                                                                    ...
                                                                    <div className="min-w-0">
                                                                        ...
                                                                        {req.status?.toLowerCase() !== 'pending' ? (
                                                                            <span className={`text-[10px] uppercase font-bold ${req.status?.toLowerCase() === 'accepted' ? 'text-green-600' : 'text-red-500'}`}>
                                                                                {req.status}
                                                                            </span>
                                                                        ) : (
                                                                ...
                                                                <div className="flex items-center gap-2 shrink-0">
                                                                    {req.status === "pending" ? (
                                                                        <>
                                                                            <button onClick={() => handleAction(req.id, "accepted")} className="p-2 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition border border-green-200 cursor-pointer shadow-sm" title="Accept">
                                                                                <FiCheck />
                                                                            </button>
                                                                            <button onClick={() => handleAction(req.id, "rejected")} className="p-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition border border-red-200 cursor-pointer shadow-sm" title="Reject">
                                                                                <FiX />
                                                                            </button>
                                                                        </>
                                                                    ) : (
                    ...
                    <div className="space-y-6">
                        {joinedTrips.length === 0 ? (
                            <div className="text-center py-16 border-2 border-dashed border-travel-border rounded-xl bg-travel-card">
                                ...
                                <p className="text-travel-text-muted mb-6">Find an upcoming adventure and ask to join!</p>
                                <Link href="/" className="bg-travel-accent text-white px-6 py-2 rounded-full font-medium hover:bg-travel-accent-hover transition shadow-sm">
                                    Explore Feed
                                </Link>
                            </div>
                        ) : (
                            joinedTrips.map((request) => (
                                <div key={request.id} className="bg-travel-card rounded-xl shadow-sm border border-travel-border p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition hover:shadow-md">
                                    <div className="flex flex-col gap-2">
                                        <div className="flex items-center gap-3">
                                            <h3 className="text-2xl font-bold text-travel-text">
                                                {typeof request.trip.destination === 'string' ? request.trip.destination : request.trip.destination?.name}
                                            </h3>
                                            <span className={`px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full ${request.status?.toLowerCase() === 'accepted' ? 'bg-green-100 text-green-700 border border-green-200' :
                                                request.status?.toLowerCase() === 'rejected' ? 'bg-red-100 text-red-700 border border-red-200' :
                                                    'bg-orange-100 text-orange-700 border border-orange-200'
                                                }`}>
                                                {request.status}
                                            </span>
                                        </div>
                                        <p className="text-sm text-travel-text-muted flex items-center gap-2">
                                            <FiCalendar className="text-travel-accent" />
                                            {formatDate(request.trip.startDate)} — {formatDate(request.trip.endDate)}
                                        </p>
                                        <p className="text-sm font-medium text-travel-text mt-1">
                                            Hosted by: <span className="text-travel-accent">{request.trip.user.name}</span>
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3 w-full md:w-auto">
                                        <Link
                                            ...
                                            className="flex-1 md:flex-none text-center px-6 py-2 bg-travel-bg border border-travel-border hover:border-travel-accent text-travel-text font-bold rounded-lg transition-colors cursor-pointer"
                                        >
                                            View Trip
                                        </Link>
                                        {/* If Accepted, show the mail icon to quickly email the Host! */}
                                        {request.status?.toLowerCase() === 'accepted' && request.trip.user.email && (
                                            <a href={`mailto:${request.trip.user.email}`} className="p-2.5 bg-travel-accent text-white rounded-lg hover:bg-travel-accent-hover transition shadow-sm" title="Email Host">
                                                <FiMail size={18} />
                                            </a>
                                        )}
                                    </div>
